...
describe('cartReducer', () => {
    ...
    it('should toggle hidden with the toggleCartHidden action', () => {
    ...
    it('should increase quantity of matching item by 1 if addItem action fired with same item as payload', () => {
        ...
        expect(cartReducer(mockPrevState, { type: CartActionTypes.ADD_ITEM, payload: mockItem}).cartItems[0].quantity).toBe(4);
    });
    ...
    it('should decrease quantity of matching item by 1 if removeItem action fired with same item as payload', () => {
        ...
        expect(cartReducer(mockPrevState, { type: CartActionTypes.REMOVE_ITEM, payload: mockItem}).cartItems[0].quantity).toBe(2);
    });
    ...
    it('should remove item from cart if clearItemFromCart action fired with payload of existing item', () => {
        ...
        expect(cartReducer(mockPrevState, { type: CartActionTypes.CLEAR_ITEM_FROM_CART, payload: mockItem}).cartItems.includes(item => item.id === 1)).toBe(false);
    });
